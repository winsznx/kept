...
11. **Convex reviewer findings (2026-09-22), all four High issues fixed:** a storage id can back only one document globally, and the claim check runs before any rejection path deletes a file; follow-up case mail replies in-thread only when the last inbound came from the approved recipient's domain and nothing bounced, otherwise it sends fresh to exactly the approved address; demo session creation is throttled globally (20/minute) and expired sessions stop resolving immediately; source jobs are limited to 40 per workspace per hour, and handled inbound messages can't be re-assigned. Deferred Medium items: return validators on the three large view queries (`items.getMine`, `cases.getMine`, `demo.state`), and routing-token auto-assignment trusting the sender (listed in the threat model).
